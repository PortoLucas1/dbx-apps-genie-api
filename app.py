...
import json
from genie_room import genie_query, get_sample_questions, get_space_info, send_feedback
import pandas as pd
import os
...
# Fetch Genie Space info with fallback to hard-coded defaults
def get_welcome_info():
    """Get welcome title and description from Genie Space, fallback to defaults"""
    # Hard-coded backup values
    FALLBACK_TITLE = "Supply Chain Optimization"
    FALLBACK_DESCRIPTION = "Analyze your Supply Chain Performance leveraging AI/BI Dashboard. Deep dive into your data and metrics."
    
    space_name, space_description = get_space_info()
    
    print(f"[DEBUG] Fetched space name: '{space_name}', description: '{space_description}'")
    
    # Use fetched values if available, otherwise fallback to hard-coded defaults
    title = space_name if space_name else FALLBACK_TITLE
    description = space_description if space_description else FALLBACK_DESCRIPTION
    
    print(f"[DEBUG] Using title: '{title}', description: '{description}'")
    
    return title, description

DEFAULT_WELCOME_TITLE, DEFAULT_WELCOME_DESCRIPTION = get_welcome_info()

# Add default suggestion questions
def get_default_suggestions():
    """Get suggestion questions from Genie Space, fallback to defaults"""
    space_questions = get_sample_questions()
    
    print(f"[DEBUG] Fetched {len(space_questions)} sample questions from Genie Space")
    
    # Use space questions if available, otherwise fallback to hardcoded defaults
    if space_questions and len(space_questions) >= 4:
        print(f"[DEBUG] Using Genie Space questions: {space_questions[:4]}")
        return space_questions[:4]  # Take first 4 questions
    
    # Fallback defaults
    print(f"[DEBUG] Using fallback defaults (only {len(space_questions)} questions found)")
    return [
...
print(f"[DEBUG] DEFAULT_SUGGESTIONS set to: {DEFAULT_SUGGESTIONS}")
...
@app.callback(
    ...
    [Input("suggestion-1", "n_clicks"),
     ...
     Input({"type": "suggested-question-btn", "index": ALL}, "n_clicks")],
    [State("suggestion-1-text", "children"),
     ...
     State({"type": "suggested-question-btn", "index": ALL}, "children")],
    prevent_initial_call=True
)
def handle_all_inputs(s1_clicks, s2_clicks, s3_clicks, s4_clicks, send_clicks, submit_clicks, suggested_clicks,
                     s1_text, s2_text, s3_text, s4_text, input_value, current_messages,
                     welcome_class, current_chat_list, chat_history, session_data, suggested_texts):
    ...
    elif trigger_id.startswith('{"type":"suggested-question-btn"'):
        # Handle suggested question button clicks
        import json
        button_id = json.loads(trigger_id)
        # Find which button was clicked (the one with clicks > 0)
        for idx, clicks in enumerate(suggested_clicks):
            if clicks and clicks > 0:
                user_input = suggested_texts[idx]
                break
        else:
...
def get_model_response(trigger_data, current_messages, chat_history):
    ...
    try:
        response, query_text, suggested_questions, conversation_id, message_id = genie_query(user_input)
        print(f"[DEBUG] Received {len(suggested_questions)} suggested questions: {suggested_questions}")
        print(f"[DEBUG] Conversation ID: {conversation_id}, Message ID: {message_id}")
        ...
        # Create suggested questions component if available
        suggested_questions_component = None
        if suggested_questions and len(suggested_questions) > 0:
            print(f"[DEBUG] Creating suggested questions UI component with {len(suggested_questions)} questions")
            suggested_questions_component = html.Div([
                html.Div("Suggested follow-up questions:", className="suggested-questions-title"),
                html.Div([
                    html.Button(
                        question,
                        id={"type": "suggested-question-btn", "index": f"{len(chat_history)}-{idx}"},
                        className="suggested-question-button",
                        n_clicks=0
                    ) for idx, question in enumerate(suggested_questions[:3])  # Limit to 3 questions
                ], className="suggested-questions-list")
            ], className="suggested-questions-container")
        else:
            print(f"[DEBUG] No suggested questions to display")
        
        # Create unique identifier for this response
        response_index = len(chat_history)
        
        # Create bot response with feedback buttons that include conversation and message IDs
        bot_response = html.Div([
            ...
            html.Div([
                content,
                suggested_questions_component if suggested_questions_component else None,
                html.Div([
                    html.Div([
                        html.Button(
                            id={
                                "type": "thumbs-up-button", 
...
# Fix the callback for thumbs up/down buttons - Now with API feedback
@app.callback(
    [Output({"type": "thumbs-up-button", "index": MATCH, "conversation_id": MATCH, "message_id": MATCH}, "className"),
     Output({"type": "thumbs-down-button", "index": MATCH, "conversation_id": MATCH, "message_id": MATCH}, "className")],
    [Input({"type": "thumbs-up-button", "index": MATCH, "conversation_id": MATCH, "message_id": MATCH}, "n_clicks"),
     Input({"type": "thumbs-down-button", "index": MATCH, "conversation_id": MATCH, "message_id": MATCH}, "n_clicks")],
    [State({"type": "thumbs-up-button", "index": MATCH, "conversation_id": MATCH, "message_id": MATCH}, "className"),
     State({"type": "thumbs-down-button", "index": MATCH, "conversation_id": MATCH, "message_id": MATCH}, "className")],
    prevent_initial_call=True
)
def handle_feedback(up_clicks, down_clicks, up_class, down_class):
    ...
    conversation_id = button_data.get("conversation_id")
    message_id = button_data.get("message_id")
    
    # Send feedback to API if we have the required IDs
    if conversation_id and message_id:
        feedback_type = "positive" if button_type == "thumbs-up-button" else "negative"
        success = send_feedback(conversation_id, message_id, feedback_type)
        
        if success:
            print(f"[DEBUG] Successfully sent {feedback_type} feedback to API")
        else:
            print(f"[DEBUG] Failed to send {feedback_type} feedback to API")
    else:
        print(f"[DEBUG] Cannot send feedback - missing conversation_id or message_id")
